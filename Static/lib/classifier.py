...
class GadgetClassifier(object):
  ...
  def get_new_ip_from_potential_gadget(self, state, possible_types):
    ...
    for (gadget_type, inputs, outputs, params, clobber) in possible_types:
      ...
      if gadget_type == LoadMem and outputs[0] == self.ip and inputs[0] in state.in_regs:
        ip_from_reg = inputs[0]
  ...
  def is_gadget(self, code, address):
    '''whether instructions are gadget'''
    gadget = self.create_gadgets_from_instructions(code, address)
    if len(gadget) == 0:
      self.logger.debug("Not gadget")
    return gadget
  ...
  def create_gadgets_from_instructions(self, code, address):
    ...
    ip_in_stack_offset, ip_from_reg = self.get_new_ip_from_potential_gadget(state, possible_types)
    ...
    for (gadget_type, inputs, outputs, params, clobber) in possible_types:
      if (
        #all the gadgets must load rip from other registers or the stack
        (ip_from_reg == None and ip_in_stack_offset == None)

        # Ignore the LoadMem gadget for the IP register
        or (len(outputs) > 0 and outputs[0] == self.ip and gadget_type != Jump)
